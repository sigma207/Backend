IF EXISTS(select * FROM sys.views where name = 'symbol_tradable_daily_temp')
	drop view symbol_tradable_daily_temp
go

CREATE VIEW symbol_tradable_daily_temp AS
	...
	FROM (
		...
		LEFT JOIN symbol_stock_daily_temp s ON s. exchange_id = a .exchange_id AND s.symbol_id = a. symbol_id
